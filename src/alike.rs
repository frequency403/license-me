...
pub fn is_alike<T>(target: T, comparison: T, min_percent_to_reach: isize) -> (bool, isize) where T: Display { let p1string = target.to_string();
    ...
    (percent >= min_percent_to_reach as f64, percent.round() as isize)
}